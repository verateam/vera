...
BOOST_PYTHON_MODULE(vera)
{
  ...
  py::class_<Structures::Token>("Token")
      .def(py::init<>())
      .def(py::init<std::string, std::string, int, int, std::string>())
      // .def("__repr__", &Token::toString)
      .add_property("file", &Structures::Token::file_)
      .add_property("raw", &Structures::Token::raw_)
...
void PythonInterpreter::execute(const std::string& fileName)
{
  try
  {
    ...
    boost::format fmt = boost::format("sys.path.append(\"%1%\")") % \
